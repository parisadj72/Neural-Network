...
%% Load TrainSet
direction = fileparts(which('Q_5_1.m'));
...
a = cell([size(trainSet,1)*size(trainSet,2) 10]);
...
for i = 1:size(trainSet,2)
    for j = 1:size(trainSet,1)
        ...
        ab = repmat(floor(sz/10),[1 10]);
        ab(end) = floor(sz/10)+(sz-10*floor(sz/10));
        b = trainSet{j,i};
...
a_tst = cell([size(testSet,1)*size(testSet,2) 10]);
...
for i = 1:size(testSet,2)
    for j = 1:size(testSet,1)
        ...
        ab_tst = repmat(floor(sz_tst/10),[1 10]);
        ab_tst(end) = floor(sz_tst/10)+(sz_tst-10*floor(sz_tst/10));
        b_tst = testSet{j,i};
...
%% Power
power = zeros(size(trainSet,1)*size(trainSet,2),10);
power_tst = zeros(size(testSet,1)*size(testSet,2),10);
